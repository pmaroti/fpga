...
LAB_WARM 	= $30		; BASIC warm start entry point
Wrmjpl 		= LAB_WARM+1; BASIC warm start vector jump low byte
Wrmjph 		= LAB_WARM+2; BASIC warm start vector jump high byte

Usrjmp		= $3A		; USR function JMP address
Usrjpl		= Usrjmp+1	; USR function JMP vector low byte
Usrjph		= Usrjmp+2	; USR function JMP vector high byte
Nullct		= $3D		; nulls output after each line
TPos		= $3E		; BASIC terminal position byte
TWidth		= $3F		; BASIC terminal width byte
Iclim		= $40		; input column limit
Itempl		= $41		; temporary integer low byte
...
Ram_top		= ini	; end of user RAM+1 (set as needed, should be page aligned)
...
ini:	.byte	$B0,$CA
	...
	.word	boot
	.word	fini-boot
;	init hardware and relocate code
; Setup system things
boot:	jsr	$FF0F	;cls
	jmp	LAB_COLD
...
TabLoop:
	...
	STA	LAB_WARM,x	;PLUS_0,X		; save byte in page zero
	DEX				; decrement count
...
PG2_TABS:
	...
	.word	$FF2A		; non halting key input	-	monitor to set this
	.word	$FF24		; output vector		-	monitor to set this
