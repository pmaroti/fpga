...
; Enhanced BASIC to assemble under 6502 simulator, $ver 2.10

; $E7E1 $E7CF $E7C6 $E7D3 $E7D1 $E7D5 $E7CF
...
tmp1		= $DE		; UART emulation	; J. Arias
rxd		= $DF		; UART emulation (IRQ, received data)
rxf		= $E0		; UART emulation (IRQ, flag)
rxt		= $E1		; UART emulation (IRQ, temporary shift reg)
;		= $E2		; unused
;		= $E3		; unused
;		= $E4		; unused
;		= $E5		; unused
...
IRQ_vec	= VEC_SV+2		; IRQ code vector (from monitor)
NMI_vec	= IRQ_vec+$0A		; NMI code vector (from monitor)
...
;----------------------------------------------------------------------
; Setup system things
	lda	#1	; Disable ROM (J. Arias)
	ora	VIA+3	; DDRA
	sta	VIA+3	
	
	; Cambiamos vector de interrupcion
	...
	lda	#0
	sta	rxf	; Dato rx vacio
	lda	#$82
	sta	VIA+14	; (IER), Habilitamos interr CA1 (en bajada)
...
LAB_REM:
	JSR	LAB_SNBL		; scan for next BASIC line
	BEQ	LAB_170F		; go set BASIC execute pointer and return, branch always
...
PG2_TABS:
	...
	.word	uart_getchar_nb		; non halting key input	-	monitor to set this
	.word	uart_putch		; output vector		-	monitor to set this
	.word	crts			; load vector		-	monitor to set this
	.word	crts			; save vector		-	monitor to set this
PG2_TABE:
...
LAB_SMSG:
	...
	.byte	"Enhanced BASIC 2.10",$0A,$00
...
AA_end_basic:
	...
	RXD  = 3	; datos de entrada por PA3
	TXD  = 4	; datos de salida  por PA4
...
uart_putch:
	...
	ldy 	#9
	sec			; el carry será el bit de stop
	ldx 	#16
	lda 	#(1<<TXD)
	ora 	DDRA
	sta 	DDRA		; bit de start
uatx1:	dex
	bne 	uatx1		; 5*16=80 ciclos
uatx5:	lda 	tmp1		; 3 ciclos
	ror 	tmp1		; 5 ciclos
	bcs 	uatx2		; 3 ciclos
	lda 	#(1<<TXD)	; 2 ciclos
	ora 	DDRA		; 4 ciclos
...
uatx2:	lda 	#~(1<<TXD)
	...
	sta 	DDRA
	bcs 	uatx3	
uatx3:	ldx 	#14		; 2 ciclos
uatx4:	dex		
	bne 	uatx4		; 5*14=70 ciclos
	lda 	tmp1		; 3 ciclos (retardo)
	dey			; 2 ciclos
	bne 	uatx5		; 3 ciclos
...
;---------------------------------------------------------------------------
; Interrupcion en flanco de bajada de CA1 (bit de start)
...
irqbrk:
	...
	ldx	#8		;2 ciclos
	ldy	#24		;2 ciclos La primera vez retardo de 1.5 bits

uarx2:	dey			;2 ciclos
	bne	uarx2		;3 ciclos, el bucle tarda 4 x Y (80 ciclos)
	
	nop			;3 ciclos (relleno)
	nop			;2 ciclos (relleno)
	
	ldy	#16		;2 ciclos 
	
	lda	IRA		;4 ciclos (ademas borra IRQ)
	and	#(1<<RXD)	;2 ciclos
	sbc	#1		;2 ciclos
	...
	lda	#(1<<RXD)
uarx3:	bit	IRA		;esperamos por bit stop
	beq	uarx3
	...
	lda	VIA+1		;(ORA) borramos flag

irq2:		; Not from CA1 (Via)
irq3:		; Not from VIA
	pla
	tay
	pla
